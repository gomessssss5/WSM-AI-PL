...
import { ChevronDown, ArrowLeft, Scale, AlertTriangle } from 'lucide-react';
...
function ComparisonArrow({ 
  ...
  const barFillClass = isGreen ? 'bg-green-500' : 'bg-red-500';
  const textLeftClass = bubblePosition === 'left' ? (isGreen ? 'text-green-600 font-bold' : 'text-red-600 font-bold') : 'text-gray-700 font-medium';
  const textRightClass = bubblePosition === 'right' ? (isGreen ? 'text-green-600 font-bold' : 'text-red-600 font-bold') : 'text-gray-700 font-medium';
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col items-center gap-1.5 my-3 px-4">
      ...
      {centerText && (
        <span className={`text-[13px] font-bold ${isGreen ? 'text-green-600' : 'text-red-500'} bg-white px-2 py-0.5 rounded-md shadow-xs border border-gray-100`}>
          {centerText}
        </span>
      )}
      <div className="w-full flex items-center justify-between gap-4">
        ...
        <div className="flex-1 h-3 bg-gray-200/70 rounded-full relative flex items-center">
          ...
          {/* Arrowhead or bubble */}
          <div className={`absolute ${bubblePosition === 'right' ? 'right-0 translate-x-1/2' : 'left-0 -translate-x-1/2'} flex items-center justify-center`}>
            <div className={`w-6 h-6 ${barFillClass} rounded-full flex items-center justify-center shadow-md border-2 border-white animate-pulse`}>
...
export default function BenchmarkPage() {
  const [selectedBenchmark, setSelectedBenchmark] = useState('Benchmark 02');
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-screen bg-[#faf9f6] text-gray-800 font-sans overflow-y-auto">
      ...
      {/* Main Content */}
      <main className={`flex-1 w-full ${selectedBenchmark === 'Benchmark 02' ? 'max-w-7xl xl:max-w-[1400px]' : 'max-w-4xl'} mx-auto px-5 py-10 md:py-16 flex flex-col items-center transition-all duration-300`}>
        ...
        <div className="flex flex-col items-center gap-2.5 mb-12">
          ...
          <span className="text-[14px] text-gray-500 font-medium animate-in fade-in duration-300">
            {selectedBenchmark === 'Benchmark 01' ? '18 de Julho de 2026' : '19 de Julho de 2026'}
          </span>
        </div>
        ...
        <div className="w-full flex flex-col gap-12">
          {categories.map((category, index) => {
            ...
            // Benchmark 02 Comparison Layout
            return (
              <React.Fragment key={category.title}>
