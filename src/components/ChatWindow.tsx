                ...
                <div ref={attachContainerRef} className="flex items-center gap-1 relative">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-bold transition-all cursor-pointer ${
                      isSearchEnabled
                        ? 'bg-blue-600 text-white dark:bg-blue-600 dark:text-white border border-blue-600 shadow-sm ring-2 ring-blue-500/20'
                    ...
                    title={isSearchEnabled ? "Busca Web ativada (Clique para desativar)" : "Pesquisar na Web (Clique para ativar)"}
                  >
                    <Globe className={`w-3.5 h-3.5 ${isSearchEnabled ? 'text-white animate-spin-slow' : 'text-gray-500 dark:text-gray-400'}`} />
                    <span>{isSearchEnabled ? "Pesquisa Ativa" : "Pesquisar"}</span>
                  </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 scrollbar-thin scrollbar-thumb-gray-200">
              {(() => {
                ...
                return cleanList.map((src, idx) => {
                  ...
                  const isInvalid = !hostname || hostname.includes(" ") || !hostname.includes(".") || hostname.length < 3; const favUrl = isInvalid ? "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='%23666' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><line x1='2' y1='12' x2='22' y2='12'/></svg>" : `https://www.google.com/s2/favicons?domain=${encodeURIComponent(hostname)}&sz=32`;
                  return (
                    <div 
                      key={idx} 
