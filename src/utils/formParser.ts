...
export function extractWsmForm(text: string | undefined): { cleanText: string, formObj: WsmForm | null } {
  ...
  // Regex to match opening and closing tags for form/forms/wsm_form/wsm_forms (case-insensitive)
  const regex = /<(wsm_form|wsm_forms|forms|form)(?:\s+[^>]*)?>([\s\S]*?)<\/\1>/i;
  const match = regex.exec(text);

  if (!match) {
    // If we can't find a complete match, check if there's an opening tag but no closing tag yet (streaming)
    const openRegex = /<(wsm_form|wsm_forms|forms|form)(?:\s+[^>]*)?>/i;
