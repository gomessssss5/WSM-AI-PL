...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f8fafc; color: #1e293b; margin: 0; padding: 24px 12px;">
  <div style="max-width: 680px; margin: 0 auto; background: #ffffff; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
    ...
    <div style="background-color: #2563eb; color: #ffffff; padding: 22px 24px;">
      ...
      <p style="margin: 6px 0 0 0; font-size: 12.5px; opacity: 0.9; color: #ffffff;">${subtitleText || `WSM 1.6 Pro • ${nowFormatted}`}</p>
    </div>
    ...
    <div style="background-color: #f8fafc; padding: 16px 24px; text-align: center; font-size: 12px; color: #64748b; border-top: 1px solid #e2e8f0;">
      Este e-mail foi enviado automaticamente pelo WSM 1.6 Pro para ${toEmail}.
    </div>
...
`;
  ...
  try {
    ...
    const fromName = process.env.SMTP_FROM_NAME || 'WSM 1.6 Pro';
    ...
    if (resendApiKey) {
      try {
        ...
        const fromEmail = process.env.RESEND_FROM_EMAIL || 'WSM 1.6 Pro <onboarding@resend.dev>';
...
export async function sendWelcomeEmail(toEmail: string, displayName?: string) {
  ...
  return sendGenericEmail({
    ...
    badgeText: 'Boas-Vindas ao WSM 1.6 Pro',
...
export async function sendInactivityEmail(toEmail: string, daysInactive: number) {
  let subject = `[WSM 1.6 Pro] Volte a criar com o WSM 1.6 Pro`;
  let title = `O que você quer criar hoje?`;
  let body = `Notamos que faz ${daysInactive} dias desde o seu último acesso ao WSM 1.6 Pro. O que gostaria de pesquisar ou criar hoje?`;

  if (daysInactive === 10) {
    subject = `[WSM 1.6 Pro] Sentimos sua falta no WSM 1.6 Pro`;
    title = `Sentimos sua falta!`;
    body = `Faz **10 dias** que você não conversa com o WSM 1.6 Pro. Venha conferir as novas atualizações e dar sequência às suas ideias!`;
  } else if (daysInactive === 15) {
    subject = `[WSM 1.6 Pro] Seu assistente IA está te esperando`;
    title = `Seu assistente IA está te esperando!`;
    body = `Há **15 dias** sem utilizar o WSM 1.6 Pro. Lembre-se de que você pode agendar tarefas automáticas e pesquisar em tempo real a qualquer momento.`;
  } else if (daysInactive === 30) {
    subject = `[WSM 1.6 Pro] Faz 1 mês que não conversamos!`;
    title = `Faz 1 mês que não nos falamos!`;
    body = `Faz **30 dias** desde o seu último acesso ao WSM 1.6 Pro. Seu histórico de conversas e preferências continuam salvos e prontos para quando você precisar!`;
  } else if (daysInactive === 60) {
    subject = `[WSM 1.6 Pro] Que tal dar uma nova chance ao WSM 1.6 Pro?`;
    title = `Que tal dar mais uma chance ao WSM 1.6 Pro?`;
    body = `Faz **60 dias** desde a sua última interação. Experimente nossas ferramentas de navegação web e veja como o WSM 1.6 Pro evoluiu para te ajudar!`;
  } else if (daysInactive >= 150) {
    subject = `[WSM 1.6 Pro] E aí, esqueceu de mim?`;
    title = `E aí, esqueceu de mim?`;
    body = `E aí, esqueceu de mim? Faz **150 dias** que você não entra no WSM 1.6 Pro. Estou com saudades de te ajudar no dia a dia. Que tal voltar hoje mesmo e testar o que há de novo?`;
  }
...
export async function sendInterruptedResponseEmail(toEmail: string, userPrompt: string, aiResponseSnippet: string) {
  return sendGenericEmail({
    toEmail,
    subject: `[WSM 1.6 Pro] Sua resposta está pronta no WSM 1.6 Pro`,
...
A resposta completa para a sua pergunta já foi processada com sucesso e está disponível no seu histórico do WSM 1.6 Pro!

### 📝 Trecho da resposta gerada:
${aiResponseSnippet.substring(0, 1000)}${aiResponseSnippet.length > 1000 ? '...' : ''}

Acesse o WSM 1.6 Pro para ver o conteúdo completo no seu chat.
`
  });
}
...
export async function sendFeatureHighlightEmail(toEmail: string) {
  return sendGenericEmail({
    ...
    badgeText: 'Dica de Recurso do WSM 1.6 Pro',
    title: `O WSM 1.6 Pro navega diretamente em sites da web!`,
    bodyMarkdown: `
Passando para te lembrar de um dos recursos mais poderosos do **WSM 1.6 Pro**:

Diferente de IAs estáticas com dados desatualizados, o **WSM 1.6 Pro** navega diretamente na web, acessa links, pesquisa notícias do dia e interage com sites em tempo real para te fornecer respostas precisas e confiáveis.
...
export async function sendMonthlyCampaignEmail(toEmail: string) {
  return sendGenericEmail({
    ...
    badgeText: 'WSM 1.6 Pro vs Outras IAs',
